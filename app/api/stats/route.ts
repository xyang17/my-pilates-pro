...
// 计算某个周期类型 + 偏移量对应的起止日期和展示用的 label
function getRange(type: PeriodType, offset: number) {
  const now = new Date()
...
function buildTrend(rows: ClassRow[], type: PeriodType, start: Date, end: Date): Bucket[] {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { start, end, label } = getRange(type, offset)
    const startStr = toDateStr(start)
    const endStr = toDateStr(end)

    let query = supabaseAdmin
      .from('class')
      .select('id, date, price, duration, status, class_type, created_by')
      .gte('date', startStr)
      .lte('date', endStr)
    ...
    const trend = buildTrend(rows, type, start, end)
